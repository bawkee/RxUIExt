...
public static class RxUIExtensions
{
    ...
    public static IObservable<Unit> WhenExecuting(this IReactiveCommand cmd) =>
        cmd.IsExecuting
           .PairWithPrevious()
           .Where(p => p.Item1 == false && p.Item2)
           .Select(_ => Unit.Default);

    /// <summary>
    /// An idiomatic WhenActivated which combines view model and activation logic, crucial when you expect view model to change 
    /// on a single view.
    /// </summary>
    ...
    /// <summary>
    /// Makes sure that all Observable exceptions are propagated to RxApp.DefaultExceptionHandler.
    /// </summary>
